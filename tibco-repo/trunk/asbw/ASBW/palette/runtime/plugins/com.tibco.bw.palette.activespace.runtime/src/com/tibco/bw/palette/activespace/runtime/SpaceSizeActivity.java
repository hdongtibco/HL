...
public class SpaceSizeActivity<N> extends ASSpaceSizeBaseActivity<N> {

	@Property
	public SpaceSize activityConfig;
    /**
	...
	class ASSpaceSizeExecutor<A> implements Runnable {
		...
		public void run() {
			...
			try {
				...
				result = invokeActiveSpaceAPI(filter, processContext);
	...
	protected N invokeActiveSpaceAPI(String filter, ProcessContext<N> processContext) throws Exception {
		try {
			Space space = spaceConnectionResource.getSpace();
			long size = 0;
			if(filter != null) {
				size = space.size(filter);
			} else {
				size = space.size();
			}
...
//		mutableModel.appendChild(result, size);
		...
		mutableModel.appendChild(outputType, size);
		return outputType;
	}
