...
public class RollbackTransactionActivity<N> extends TransactionBaseActivity<N> {
	
	@Property
	public RollbackTransaction activityConfig;
	...
	public void execute(N input, ProcessContext<N> processContext, AsyncActivityController asyncActivityController) throws ActivityFault {
		...
		Future task = threadPool.submit(new RollbackTransactionExecutor<N>(notifier, asContextProcessResource));
	...
	public N postExecute(Serializable value, ProcessContext<N> processContext) throws ActivityFault {
		...
		if(getActivityLogger().isDebugEnabled()){
			activityLogger.debug("\nExit of Activity RollbackTransactionActivity");
		}
		return null;
	}
    
	class RollbackTransactionExecutor<A> implements Runnable {

		private AsyncActivityCompletionNotifier notifier = null;
		private ASContextProcessResource asContextProcessResource = null;
		
		public RollbackTransactionExecutor(AsyncActivityCompletionNotifier notifier, ASContextProcessResource asContextProcessResource) {
		...
		public void run() {
			...
			activityLogger.debug("\nStart of Activity RollbackTransactionActivity");
			...
			try {
				if (asContextProcessResource == null) {
					throw new ASException(ASStatus.SYS_ERROR, "No RollbackTransactionActivity found");
				} else {
