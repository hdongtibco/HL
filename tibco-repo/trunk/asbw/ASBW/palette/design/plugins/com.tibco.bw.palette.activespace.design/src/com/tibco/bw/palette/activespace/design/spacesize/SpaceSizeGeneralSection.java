 ...
 */
public class SpaceSizeGeneralSection extends AbstractBWTransactionalSection {
   ...
   protected Class<?> getModelClass() {
      return SpaceSize.class;
   }

   @Override
   protected void initBindings() {
	   getBindingManager().bind(spaceConnection, AsPackage.Literals.SPACE_SIZE__SPACE_CONNECTION, getInput(), BWFieldFactory.getInstance().getPropertyTargetToModelUpdateValueStrategy(), null);
   }

   @Override
   protected Composite doCreateControl(final Composite root) {
      Composite parent = BWFieldFactory.getInstance().createComposite(root, 2);
   	  BWFieldFactory.getInstance().createLabel(parent, Messages.ASSpaceSize_SPACE_CONNECTION_LABEL, true);
