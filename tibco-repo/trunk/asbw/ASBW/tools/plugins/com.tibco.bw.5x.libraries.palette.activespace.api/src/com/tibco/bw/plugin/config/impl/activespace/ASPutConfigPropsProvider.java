...
import com.tibco.bw.plugin.config.activespace.ASPutConfigProps;
...
public class ASPutConfigPropsProvider extends DefaultConfigPropsProvider
		implements ASPutConfigProps, ASExpandedNameConstants {

	public ASPutConfigPropsProvider(ActivityDescriptor paramActivityDescriptor) {
	...
	public String getPropertyValueAsString(byte paramByte) {
		...
		} else if (paramByte == TIME_TO_LIVE_BYTE) {
			str = XiChild.getString(localXiNode, TTL_EN, null);
	...
	public boolean getPropertyValueAsBoolean(byte paramByte) {
		...
		if (paramByte == FORGET_BYTE) {
			...
			bool = XiChild.getBoolean(localXiNode, IS_COMPARE_AND_PUT_EN, false);
